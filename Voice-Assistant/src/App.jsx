...
const speakText = (text)=>{
   ...
   const maleEnglishVoice = voices.find((voice)=>
    voice.lang.startsWith("en-") && voice.name.toLowerCase().includes("male")) || voices.find((voice)=>voice.lang.
  startsWith("en-")) || voices[0]
...
const handleVoiceCommand = async(command)=>{
  ...
  if(command.includes("what is your name")){
    const response = "Hello sir I'm AI Expert, Your voice assistant created by Balkeshwar";
    speakText(response);
    setInformation(response);
    return;
  }else if(command.includes("Hello AI")){
    const response = "Hello sir I'm AI Expert, How can i help you";
    speakText(response);
    setInformation(response);
    return;
  }else if(command.includes("what is your age")){
    const response = "Hello sir I'm AI Expert, I'm 1 month old";
    speakText(response);
    setInformation(response);
    return;
  }else if(command.includes("Who are you")){
    const response = "Hello sir I'm  your AI Expert voice assistant";
    speakText(response);
    setInformation(response);
    return;
  }
